...
exports.updateTaskStatus = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Внутренняя ошибка сервера при обновлении статуса' });
  }
};
...
exports.deleteTask = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Задача успешно удалена', deletedTask: result.rows[0] });
  } catch (error) {
